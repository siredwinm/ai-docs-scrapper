...
def discover_llms(
    ...
    for llms_url in unique_urls(candidates, len(candidates)):
        try:
            text = fetch(session, llms_url, timeout).text
...
def discover_sitemap(
    ...
    while sitemap_urls and len(discovered) < max_pages:
        ...
        try:
            response = fetch(session, sitemap_url, timeout)
...
def discover_crawl(
    ...
    while queue and len(result) < max_pages:
        ...
        try:
            html = fetch(session, url, timeout).text
        except requests.RequestException:
            continue
...
def scrape_page(session: requests.Session, url: str, timeout: float) -> tuple[str, str]:
...
def write_outputs(
    ...
    for index, url in enumerate(urls, start=1):
        try:
            title, markdown = scrape_page(session, url, timeout)
...
def main(argv: list[str] | None = None) -> int:
    ...
    seed_url = require_http_url(args.url) if args.url else ""
    base_url = args.base_url or (default_base_url(seed_url) if seed_url else origin_for(url_file_items[0]))
    base_url = require_http_url(base_url, "--base-url")

    if args.urls_file:
        urls = url_file_items
    elif args.mode == "llms":
        urls = discover_llms(session, seed_url, base_url, args.timeout, args.max_pages)
    elif args.mode == "sitemap":
        urls = discover_sitemap(session, seed_url, base_url, args.timeout, args.max_pages)
    elif args.mode == "crawl":
        urls = discover_crawl(session, seed_url, base_url, args.timeout, args.max_pages)
    elif args.mode == "url-list":
        urls = [seed_url]
    else:
        urls = (
            discover_llms(session, seed_url, base_url, args.timeout, args.max_pages)
            or discover_sitemap(session, seed_url, base_url, args.timeout, args.max_pages)
            or discover_crawl(session, seed_url, base_url, args.timeout, args.max_pages)
        )
